...
def test_open_lot_by_clerk(socket_clerk_client):
    ...
    event_received_by_clerk.wait(timeout=20)  # Adjust the timeout value as needed
...
def test_biding_with_2_customers_and_receiving_events_with_customers_and_clerk(socket_clerk_client,
    ...
    body_bid_user1 = {"SaleID": 1369, "LotID": 400525, "LotNumber": 6, "BidValue": 10000,
                      "PaddleNumber": data['first_user']['paddleNum'],
                      "BidSubmitType": "LIVE"}
    body_bid_user2 = {"SaleID": 1369, "LotID": 400525, "LotNumber": 6, "BidValue": 9000,
                      "PaddleNumber": data['second_user']['paddleNum'],
                      "BidSubmitType": "LIVE"}
    ...
    # Wait for the event to be received or timeout after a certain duration
    event_received_by_clerk.wait(timeout=20)  # Adjust the timeout value as needed
    event_received_by_firstCustomer.wait(timeout=20)  # Adjust the timeout value as needed
    event_received_by_secondCustomer.wait(timeout=20)  # Adjust the timeout value as needed
...
def test_finding_the_winner_befor_retract(socket_clerk_client):
    ...
    event_received_by_first_customer.wait(timeout=20)
...
def test_retract_request_for_winner_customer_by_customer(socket_firstCustomer_client):
    received_events_by_first_customer = []
    event_received_by_first_customer = threading.Event()
    ...
    body = {"Live": True, "SaleID": 1369, "LotID": 400525, "Status": "REQUEST_FOR_RETRACT"}
    res = request_for_retract(body, user1_token)
    print("+++++++++++++++++++++++++", res)

    # Wait for the event to be received or timeout after a certain duration
    event_received_by_first_customer.wait(timeout=20)  # Adjust the timeout value as needed
...
def test_accept_or_reject_retract_request_by_clerk(socket_clerk_client):
    received_events_by_clerk = []
    event_received_by_clerk = threading.Event()
    ...
    clerk_list_body = {"SaleID": 1369, "LotID": 400525}
    list_res = clerkList(clerk_list_body, clerk_token)
    if list_res:
        bids = list_res.get("data", {}).get("Bids", [])
        bidID = next((bid["BidID"] for bid in bids if bid.get("UserID") == 30455), None)
    print("-----------", bidID)
    body = {
        "Live": True,
        "BidID": bidID,
    ...
    event_received_by_clerk.wait(timeout=20)
    ...
    assert received_events_by_clerk[0]['MaxBid']['UserID'] == 30455 , "the user is wrong"
...
def test_close_lot_by_clerk_and_determine_the_winner(socket_clerk_client):
    ...
    body = {"SaleID": 1369, "Event": "FINISHED_LOT_TIME", "LotID": 400525}
    res = auctioneer(body, clerk_token)
